...
function flushKeys() {
  try {
    ...
    const out = {};
    for (const [h, item] of keys) if (!item.fixed) out[h] = item;
...
if (process.env.DEMO_API_KEY) {
  keys.set(hashKey(process.env.DEMO_API_KEY), { name: 'Demo Developer', prefix: 'demo', created: Date.now(), requests: 0, fixed: true });
}

// Keys defined in Railway variables survive every redeploy (no Volume needed):
//   API_KEYS=mybot=mhg_<long random value>,friend=mhg_<another long value>
for (const entry of (process.env.API_KEYS || '').split(/[\n,]+/)) {
  const i = entry.indexOf('=');
  if (i < 1) continue;
  const name = entry.slice(0, i).trim();
  const key = entry.slice(i + 1).trim();
  if (key.length < 24) { console.warn(`API_KEYS: the key for "${name}" is shorter than 24 characters and was ignored`); continue; }
  keys.set(hashKey(key), { name, prefix: key.slice(0, 10), created: Date.now(), requests: 0, fixed: true });
}
